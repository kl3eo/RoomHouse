...
$IPTABLES -t nat -A PREROUTING -i $WLAN_IF --match multiport -p tcp ! --dport 3128,443,22,587,993,4477,5900,8080 -j REDIRECT --to-port 3128
...
$IPTABLES -t nat -A PREROUTING -i $SEC_IF --match multiport -p tcp ! --dport 3128,443,22,587,993,4477,5900,8080 -j REDIRECT --to-port 3128
...
#$IPTABLES -t nat -A POSTROUTING -o $PRI_IF -s $WLAN_DHCP -j MASQUERADE
...
#$IPTABLES -t nat -A POSTROUTING -o $PRI_IF -s $WLAN_DHCP -d 149.5.28.110 --match multiport -p tcp --dport 587,993 -j MASQUERADE
...
$IPTABLES -A OUTPUT -s $SQUID_FADDR -p tcp -j MARK --set-mark 0x01 -t mangle
$IPTABLES -t nat -A POSTROUTING -s $SQUID_FADDR -o $PRI_IF -j MASQUERADE
...
#$IPTABLES -t nat -A POSTROUTING -o $PRI_IF -s $SEC_NET -d 149.5.28.110 --match multiport -p tcp --dport 587,993 -j MASQUERADE
...
#qos ddos reco
$IPTABLES -A INPUT -p tcp --match multiport --dports 80,443,8443 -m state --state NEW -m limit --limit 60/minute --limit-burst 250 -j ACCEPT

# limits the number of established/concurrent connections:
$IPTABLES -A INPUT -p tcp --match multiport --dports 80,443,8443 -m state --state RELATED,ESTABLISHED -m limit --limit 50/second --limit-burst 50 -j ACCEPT

# limits the connections from a single source IP to 50:
$IPTABLES -A INPUT -p tcp --syn --match multiport --dports 80,443,8443 -m connlimit --connlimit-above 50 -j REJECT
...
$IPTABLES -A INPUT -d $SQUID_FADDR -i $PRI_IF -m state --state RELATED,ESTABLISHED -j ACCEPT

#for PRI:
#ssh
$IPTABLES -A INPUT -s 81.25.50.12 -p tcp -m tcp --dport 22 -j ACCEPT
$IPTABLES -A INPUT -s 91.188.188.198 -p tcp -m tcp --dport 22 -j ACCEPT

#5432
#$IPTABLES -A INPUT -s 82.146.44.218 -d $PRI -p tcp -m tcp --dport 5432 -j ACCEPT
...
$IPTABLES -A INPUT -d $PRI -p tcp -m tcp --dport 8433 -j ACCEPT

#ETH squid
$IPTABLES -A INPUT -d $PRI -p tcp -m tcp -s $PRI_NET --dport 8080 -j ACCEPT
...
$IPTABLES -A OUTPUT -o $PRI_IF -d 192.168.0.0/16 -j ACCEPT
$IPTABLES -A OUTPUT -o $PRI_IF -d $PRI -j ACCEPT
$IPTABLES -A OUTPUT -o $PRI_IF -p tcp -m mark --mark 0x01 -m tcp --dport 443 -j ACCEPT
$IPTABLES -A OUTPUT -o $PRI_IF -p tcp -m mark --mark 0x01 -m tcp --dport 80 -j ACCEPT
$IPTABLES -A OUTPUT -s $PRI -o $PRI_IF -m state --state RELATED,ESTABLISHED -j ACCEPT
