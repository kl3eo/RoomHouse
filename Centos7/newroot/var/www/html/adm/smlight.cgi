...
$admmode = "search_identifier" if ($admmode eq '' && !$is_adm);
...
theString="/?mode=uc_smlight&admmode=log_filter&sender="+s+"&receiver="+r+"&period="+p
...
</head><body style="background:#ddd;">};

#print "OK, you are $u, adm is $is_adm!, admmode is $admmode!<br>";

if ($admmode eq 'Stats' || $admmode eq '') {

print qq{<form method="get" action="/">};
print qq{<div id=container style="padding:10px;">
	...
	<div style="float:left;margin-left:20px;"><input type=submit name=admmode value=Stats></div>
	<div style="float:left;margin-left:20px;"><input type=submit name=admmode value=Daemon></div>
	<div style="clear:left;"><input type=hidden name=mode value=uc_smlight></div>
</div>
};
...
close (IN);
...
} elsif ($admmode eq 'log_filter' || $admmode eq 'search_identifier') {
	...
	print "Пробуем фильтр на точное совпадение sender <b>$sender</b>, receiver <b>$receiver</b>!<br><br>"  if ($admmode eq 'log_filter');
	print "Пробуем фильтр на точное совпадение <b>$identifier</b>!<br><br>"  if ($admmode eq 'search_identifier');
	print qq{
	<div>
		...
		<form method="get" action="/">
			<div style="float:left;margin-left:5px;">
				...
				<input type=submit name=admmode value=search_identifier>
			</div>
			<div style="float:left;margin-left:20px;"><input type=submit name=admmode value=Daemon></div>
			<div style="clear:left;"><input type=hidden name=mode value=uc_smlight></div>
		</form>
...
print qq{<form method="get" action="/" onsubmit="alert('restarting..');">};
print qq{<div id=container>
	<div style="float:left;margin-left:120px;width:240px;">Перезапуск демона в тех случаях,<br>когда для этого <b>есть основания</b>!</div>
	<div style="float:left;margin-left:20px;">
		<input type=hidden name=rst value=1>
		<input type=submit name=admmode value=Daemon></div>
		<div style="clear:left;"><input type=hidden name=mode value=uc_smlight></div>
</div>
};
