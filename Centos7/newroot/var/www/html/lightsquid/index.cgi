...
foreach $yearname (sort keys %yearhash) {
    ...
    $yearurl_L= qq(/cgi/genc/cp?mode=ls_panel&year=$yearname&month=all);
...
for ($i=1;$i<=12;$i++) {
    ...
    $monthurl_L=(defined $monthhash{$monthtmp} )?"/cgi/genc/cp?mode=ls_panel&year=$year\&month=$monthtmp":"";
    $monthurl_B="$monthtmp";
...
foreach $daypath (@days) {
    ...
    $daydate_L ="/cgi/genc/cp?mode=ls_day_detail&$urldate";  $daydate_B ="$printdate";
    $daygroup_L="/cgi/genc/cp?mode=ls_group_detail&$urldate";$daygroup_B="##MSG_GRP##";
...
$printtotalsize_L = "/cgi/genc/cp?mode=ls_month_detail&year=$year&month=$month";
...
ReplaceTPL_URL(  TOP_MONTH,"/cgi/genc/cp?mode=ls_topsites&year=$year&month=$month&mmode=month",    "##MSG_MONTH##");
ReplaceTPL_URL(  TOP_YEAR ,"/cgi/genc/cp?mode=ls_topsites&year=$year&month=$month&mmode=year" ,    "##MSG_YEAR##");
ReplaceTPL_URL(TOTAL_MONTH,"/cgi/genc/cp?mode=ls_month_detail&year=$year&month=$month",           "##MSG_MONTH##");
ReplaceTPL_URL(TOTAL_YEAR ,"/cgi/genc/cp?mode=ls_month_detail&year=$year&month=all",              "##MSG_YEAR##");
ReplaceTPL_URL(GROUP_MONTH,"/cgi/genc/cp?mode=ls_group_detail&year=$year&month=$month&mmode=month","##MSG_MONTH##");
ReplaceTPL_URL(GROUP_YEAR ,"/cgi/genc/cp?mode=ls_group_detail&year=$year&mmode=year",              "##MSG_YEAR##");
ReplaceTPL_URL(MONTH_GRAPH,"/cgi/genc/cp?mode=ls_graph&year=$year&month=$month&mmode=month",       "");
