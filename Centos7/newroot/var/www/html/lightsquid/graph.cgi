...
sub InitGraph() {
    ...
    for ( $i = 0 ; $i < $#{ $gridvalues{$mmode} } + 1 ; $i++ ) {
        $gridvalues{$mmode}[$i] =~ m/(\d*)(\D)([\.-])?/;
...
if ( $mmode eq "user" )  { $max = $graphmaxuser; $varname="graphmaxuser";}
if ( $mmode eq "month" ) { $max = $graphmaxall;  $varname="graphmaxall";}
...
if ( $png == 1 ) {
    ...
    $graphicsurl = "/cgi/genc/cpcgi/genc/cp?mode=ls_graph&png=1\&year=$year\&month=$month\&mmode=$mmode";
    $graphicsurl .= "\&user=$user" if ( $mmode eq "user" );
...
if ( $png == 1 ) {
    ...
    $url=($mmode eq "user")?"/cgi/genc/cpcgi/genc/cp?mode=ls_user_detail":"/cgi/genc/cpcgi/genc/cp?mode=ls_day_detail";
    $url.=qq(&year=$year&month=$month);
    $url.=($mmode eq "user")?"\&user=$user":"";
    $url = URLEncode($url);
