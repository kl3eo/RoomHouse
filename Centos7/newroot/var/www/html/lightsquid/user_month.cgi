...
#    $daypath =~ m/(\d\d\d\d)(\d\d)(\d\d)/;
    ...
    $daydate_L =URLEncode("/cgi/genc/cpcgi/genc/cp?mode=ls_user_detail&$urldate&user=$user");  $daydate_B ="$printdate";
...
$graphurl_L=URLEncode("/cgi/genc/cpcgi/genc/cp?mode=ls_graph&year=$year&month=$month&mmode=user&user=$user");
