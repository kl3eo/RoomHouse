...
if ($mmode eq "day") {
    ($dev,$ino,$mmode_,$nlink,$uid,$gid,$rdev,$size,$atime,$mtime,$ctime,$blksize,$blocks) = stat("$reportpath/$mask");
    ...
    $topsitesurl="/cgi/genc/cp?mode=ls_topsites&year=$year&month=$month&day=$day";
    $bigfilesurl="/cgi/genc/cp?mode=ls_bigfiles&year=$year&month=$month&day=$day";
...
#	print "\t$hUserPosition{$user}. $user\t$hUserTotal{$user}\n";
	...
	$realname=GetRealName($daypath,$user);
        ...
        if ($mmode eq "day") {
	    $url_L="/cgi/genc/cp?mode=ls_user_detail&year=$year&month=$month&day=$day&user=$user";
	} elsif ($mmode eq "month") {
	    $url_L="/cgi/genc/cp?mode=ls_user_detail&year=$year&month=$month&day=$day&user=$user&mmode=month";
	...
	$timeurl_L=URLEncode("/cgi/genc/cp?mode=ls_user_time&year=$year&month=$month&day=$day&user=$user");
...
HideTPL("dayinfo") if ($mmode ne "day");
HideTPL("timereport") if (($timereport == 0) or ($mmode ne "day"));
