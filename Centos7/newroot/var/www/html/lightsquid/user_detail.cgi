...
if ($flagbigfiles) {
    ...
    $bigfiles_L="/cgi/genc/cp?mode=ls_bigfiles&year=$year&month=$month&day=$day&user=$user";
}
#---------
...
ReplaceTPL_URL(TIMEURL,"/cgi/genc/cp?mode=ls_user_time&year=$year&month=$month&day=$day&user=$user","##MSG_TIME_LINK##");
ReplaceTPL_URL(BIGFILESURL,$bigfiles_L,$bigfiles_B);
      
ApplyTPL();
HideTPL("timereport") if (($timereport == 0) || ($mmode ne "day"));
