...
($dev,$ino,$mmode,$nlink,$uid,$gid,$rdev,$size,$atime,$mtime,$ctime,$blksize,$blocks) = stat("$daypath");
...
#  $printputpost .= " (".sprintf("%2.2f",$putpostpercent).")";
  ...
  $userurl_L=URLEncode("/cgi/genc/cp?mode=ls_user_detail&year=$year&month=$month&day=$day&user=$user");
  $userurl_B="$user";
...
#  $userurl_L =~ s/#/%23/;
  ...
  $usergroupurl_L="/cgi/genc/cp?mode=ls_group_detail&year=$year&month=$month&day=$day#$hGroup{$user}";
